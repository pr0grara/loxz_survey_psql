...
import analyze from '../../../text_analysis/analyze';
...
class Survey extends React.Component {
  ...
  async analyzer(answers) {
    this.analyzed = answers.map(answer => {
      return answer.length == 1 ? analyze([answer[0].value]) : undefined;
    })
  }

  async handleSubmit(e) {
    e.preventDefault();
    var surveyNo = (await axios.get("/api/results/count")).data.length;
    ...
    var questions = Array.from(document.querySelectorAll('div[class="survey-question"]'))
    ...
    answers.forEach((answer, idx) => {
      ...
      const newAnswer = {
        ...
        content: answer.length == 1 ? answer[0].value : answer[0].checked ? answer[0].value : answer[1].value,
        analysis: this.analyzed[idx],
        resultNo: surveyNo,
      };
      newResult.answers.push(newAnswer);
      return axios.post('/api/answers/new', newAnswer);
    })
    return axios.post('/api/results/new', newResult);
  }
